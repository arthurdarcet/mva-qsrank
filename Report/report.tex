...
\[
	r_{loss} = 1-\frac{|\text{found neighbours}|}{|\text{true neighbours}|} 
\]
...
Although, the algorithm seems to have good speed performances ($\sim 0.2s$ according to \citep{QSRank}), ours is not as fast. This could be explained by the use of the \textit{Matlab} software and the hashmap generations that seems to take too much time.
